                    ...
                    <?php if(!empty($data)): ?>
                        <?php foreach($data as $row): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="<?= config('url.admin_root'); ?>/hospital-authority/<?= $row['id']; ?>"><?= $row['name']; ?></a>
                                </td>
                                <td>
                                    <?= mb_substr($row['desc'], 0, 80); ?><?php if(mb_strlen($row['desc']) > 20) echo '...'; ?>
                                </td>
                                <td>
                                    <a href="<?= config('url.admin_root'); ?>/hospital-authority/status/<?= $row['id'].'/';if($row['status']) echo 0;else echo 1; ?>">
